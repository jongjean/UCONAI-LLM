...
const NAVIGATION_ITEMS = [
    ...
    },
    {
        ...
        children: [
            { label: '학회소식', href: 'pages/community/notice.html' },
                ...
                <div class="footer-content">
                    ...
                    <div class="footer-links">
                        ...
                        <ul class="footer-links-list">
                            ...
                            <li><a href="${basePath}pages/community/notice.html">학회소식</a></li>
                        </ul>
                    </div>
                </div>
...
function ensureUserStatusBar() {
    ...
    const statusTemplate = `
        ...
        <div class="user-status-logged-in" style="display: none;">
            ...
            <span class="status-divider">|</span>
            <button class="status-btn logout-btn" type="button" onclick="handleESGLogout(event)">로그아웃</button>
        </div>
    `.trim();
...
// 공통 로그아웃 처리 함수
// ==========================================
window.handleESGLogout = function (e) {
    if (e) {
        e.preventDefault();
        e.stopPropagation();
    }

    // 이미 확인창이 떠 있는지 확인 (중복 방지)
    if (window.isLoggingOut) return false;
    ...
    NAVIGATION_ITEMS.forEach(item => {
        ...
        if (hasChildren) {
            ...
            item.children.forEach(child => {
                // 로그인 상태에 따른 필터링 (마이페이지 드롭다운 한정)
                if (item.label === '마이페이지') {
                    if (isLoggedIn) {
                        // 로그인 시: 회원가입, 로그인 메뉴 숨김
                        if (child.label === '회원가입' || child.label === '로그인') return;
                    }
                    // 비로그인 시에도 모든 메뉴 표시 (아래 클릭 이벤트에서 처리)
                }
                ...
                // 비로그인 상태에서 보호된 메뉴 클릭 시 리다이렉트
                if (!isLoggedIn && item.label === '마이페이지' && child.label !== '회원가입' && child.label !== '로그인') {
                    childLink.onclick = (e) => {
                        e.preventDefault();
                        e.stopPropagation();
            ...
            // 로그인 상태인 경우 마이페이지 드롭다운 마지막에 로그아웃 추가
            if (isLoggedIn && item.label === '마이페이지') {
                const logoutItem = document.createElement('li');
                logoutItem.innerHTML = `<a href="#" id="logoutBtn" style="color: #e74c3c !important; font-weight: 600;"><i class="fas fa-sign-out-alt"></i> 로그아웃</a>`;
                logoutItem.querySelector('a').onclick = window.handleESGLogout;
...
async function loadHeroSlides() {
    ...
    try {
        ...
        let slidesData = response.data;
        let slides = [];

        if (slidesData && slidesData.length > 0) {
            slides = slidesData.map(post => new SlideData({
                id: post.id,
                order: post.slide_order || 999,
                image: post.image_url,
                title: post.title,
                shortTitle: post.short_title,
                description: post.content, // content field used as description
                ...
                // imageTransform from JSON string
                imageTransform: post.image_transform ? JSON.parse(post.image_transform) : null
            }));

            console.log('✅ 서버에서 슬라이드 로드:', slides.length, '개');
        } else {
            // 기본 데이터 추가 (서버에 없을 때만)
            console.log('⚠️ 서버에 슬라이드 없음. 기본값 사용.');

            // Note: We do NOT save defaults to DB automatically to avoid pollution.
            // Just render them.
            slides.push(new SlideData({
                id: 'default_001',
                order: 1,
                image: 'images/slider/slide1.jpg', // Use local or fallback
                title: '한국ESG학회',
        ...
        // SlideRenderer로 렌더링
        const basePath = getNavBasePath();
        // 이미 API에서 is_hero=true로 필터링했으나, 더블 체크
        const visibleSlides = slides.filter(s => s.isHeroVisible !== false);

        visibleSlides.forEach((slide, index) => {
            SlideRenderer.renderMainSlide(sliderContainer, slide, index === 0, basePath);
        });
        ...
        // Fallback or alert? Just log for now.
    }
}
...
async function initializeSlider() {
    ...
    // 2. 슬라이더 컨트롤 설정
    // DOM 업데이트 직후라 안전하게 setTimeout(0) 또는 requestAnimationFrame 사용 가능
    setTimeout(() => {
        ...
        // 메인페이지 위젯 초기화 (동적 데이터 바인딩)
        if (typeof initMainPageWidgets === 'function') {
            initMainPageWidgets();
        }
...
/**
 * 메인페이지 소셜 위젯 초기화 (동적 데이터 바인딩)
 */
function initMainPageWidgets() {
    console.log('📰 메인페이지 위젯 데이터 바인딩 시작...');

    const NOTICES_KEY = 'esg_notices';
    const notices = JSON.parse(localStorage.getItem(NOTICES_KEY) || '[]');

    if (notices.length === 0) {
        console.log('ℹ️ 표시할 동적 게시물이 없습니다. 정적 데이터를 유지합니다.');
        return;
    }
    ...
    // 카테고리별 컨테이너 매핑
    const containers = {
        'notice': document.getElementById('main-notice-list'),
        'news': document.getElementById('main-news-list'),
        'event': document.getElementById('main-event-list')
    };

    // 컨테이너 초기화 (데이터가 있는 카테고리만 비움)
    Object.keys(containers).forEach(cat => {
        if (containers[cat]) {
            // 해당 카테고리에 속한 게시물이 있는지 확인
            const hasData = notices.some(n => (n.categories || []).includes(cat));
            if (hasData) {
    ...
    notices.forEach(notice => {
        const categories = notice.categories || ['notice'];
        ...
        categories.forEach(cat => {
            ...
            if (container) {
                ...
                // 상세 페이지 링크 생성
                const detailLink = `${basePath}pages/community/notice.html?id=${notice.id}`;
...
// 로그아웃 버튼 이벤트 (중복 제거: auth.js 및 handleESGLogout에서 통합 관리)
// 로그아웃 버튼 이벤트 (중복 제거: auth.js 및 handleESGLogout에서 통합 관리)
/*
if (logoutBtn) {
