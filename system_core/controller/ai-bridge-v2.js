...
const CONFIG = {
    ...
    MAX_BODY_LENGTH: 512,
    CORE_API_URL: 'http://localhost:3001/job', // Targeted Core V2 (Policy Engine)
    // CORE_API_URL: 'http://localhost:3000/job', // Legacy Core (Deprecated)
...
// --- [RESTORE DUMP LOGIC] ---
try {
    if (fs.existsSync('emergency-dump.json')) {
        const raw = fs.readFileSync('emergency-dump.json', 'utf8');
        const dump = JSON.parse(raw);

        let restoredCount = 0;

        // Restore HIGH
        if (Array.isArray(dump.HIGH)) {
            dump.HIGH.forEach(job => {
        ...
        // Restore MED
        if (Array.isArray(dump.MED)) {
            dump.MED.forEach(job => {
                queueManager.push(job);
                restoredCount++;
            });
        }
        ...
        // Restore LOW
        if (Array.isArray(dump.LOW)) {
            dump.LOW.forEach(job => {
                queueManager.push(job);
                restoredCount++;
            });
        }

        Logger.info(`Restored ${restoredCount} jobs from emergency dump.`);
        // Rename dump to verify restoration success and prevent re-use
        fs.renameSync('emergency-dump.json', `emergency-dump-${Date.now()}.json.bak`);
    }
} catch (e) {
    Logger.error('Failed to restore dump', { error: e.message });
}
